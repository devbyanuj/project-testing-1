...
export default function LoginForm() {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      <h1 className="text-2xl font-semibold transition-colors duration-200" style={{ color: 'var(--color-text)' }}>
        Welcome back
      </h1>

      {error && (
        <p className="text-sm px-3 py-2 rounded-lg transition-all duration-200" style={{ color: 'var(--color-error)', backgroundColor: 'var(--color-error)' + 15 }}>
          {error}
        </p>
      )}
      ...
      <p className="text-sm text-center transition-colors duration-200" style={{ color: 'var(--color-text-secondary)' }}>
        No account?{' '}
        <a href="/signup" style={{ color: 'var(--color-primary)' }} className="font-semibold hover:underline">
          Sign up
        </a>
      </p>
    </form>
